...
def read_split_data(root: str = r"D:\car-data", val_rate: float = 0.2):
    ...
    # 获取所有类别目录
    data = [cla for cla in os.listdir(root) if os.path.isdir(os.path.join(root, cla))]
    print(f"找到的类别: {data}")
    ...
    data.sort()
    # 生成类别名称以及对应的数字索引
    class_indices = dict((k, v) for v, k in enumerate(data))
    ...
    if plot_image:
        # 绘制每种类别个数柱状图
        plt.bar(range(len(data)), every_class_num, align='center')
