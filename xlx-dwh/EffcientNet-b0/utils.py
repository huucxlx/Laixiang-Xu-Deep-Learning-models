...
def read_split_data(root: str, val_rate: float = 0.2):
    ...
    # !!! 修复路径：将类别索引保存到明确目录
    json_path = os.path.join("data/splits", "class_indices.json")
    os.makedirs(os.path.dirname(json_path), exist_ok=True)  # 创建目录
    ...
    for cla in flower_class:
        ...
        train_paths, val_paths = train_test_split(
            images,
            test_size=val_rate,
            # stratify=[image_class]*len(images),  # 单类别可省略
            random_state=0  # 与全局seed同步
        )
...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    return epoch_loss, epoch_acc  # 确保返回损失和准确率
...
def evaluate(model, data_loader, device):
    ...
    return val_loss, val_acc  # 现在返回两个值
